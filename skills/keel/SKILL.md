...
完成条件按用户选择的模式判断，不把「代码写完」「测试通过」「审查通过」或「PR/MR 已创建」当成端到端完成：

- **end-to-end**：`keel-release` 是必经环节，不可省略。调用它并取得该环节的完成证据后，才能宣称端到端完成；仅阅读 skill、列入计划或得到 check 的 PASS 都不算执行完成。校验、CI、合入和适用的部署要求只认 `keel-release`，这里不复制规则。
- **受阻**：尚未执行 release，或执行中因门禁、权限、能力或外部条件停下，报告 `BLOCKED`，明确最后完成环节、未完成环节、原因及一个下一步决策。不得静默跳过 release 或改称任务已完成。恢复时从下一合法环节续跑。
- **route / design / dev**：按各自约定的停点交付，不强制进入 release，也不因合法停在未合入状态就报端到端失败。

结束时说出当前模式、停在哪一阶段、读过哪些环节 skill，并附对应证据（验收表、审查结果、PR/MR 或合入 SHA、适用的部署结果）。用户明确缩小或取消范围时按新范围报告，不继续强推端到端。
