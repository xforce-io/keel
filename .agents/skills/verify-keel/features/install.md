...
| # | 路径 | 可判定结果 |
|---|---|---|
| A | 入口 1，scratch home 只有默认探测面 | 退出码 `0`；`$HOME/.local/share/agent-skills/library/{keel,keel-design,keel-dev,keel-how,keel-issue,keel-reflect,keel-release,keel-review,keel-ticket,keel-verify}` 均为指向本仓库 `skills/<name>` 的 symlink；`$BIN/keel` 指向本仓库 `bin/keel`；`$HOME/.agents/skills/` 不出现任何 `keel*` |
