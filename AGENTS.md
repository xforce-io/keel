...
- `keel` 只路由；环节规则只写在对应 `keel-*` skill 里，不要抄回路由器。`keel-reflect`、`keel-ticket`、`keel-start`、`keel-sync` 与 `keel-verify-maintain` 不进交付状态机。
...
- 改安装行为时跑 `python3 -m unittest tests.test_keel tests.test_keel_reflect_inspect`。
